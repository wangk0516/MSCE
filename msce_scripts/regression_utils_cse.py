...
def CE_Expansion_FitAll(ParentLattInfo, ClusterCollection, syscmd_corr, CEOptions, logfile):
    ...
    if (CEOptions['Optimizer'] in ['BFGS', 'L-BFGS-B']) and (CEOptions['LossFunction'] in ['FITALL-CSE-SmoothPairs', 'FITALL-CSE-SmoothAllClusters', 'FITALL-CSE-L1']):
        ...
        while flag_continue:
            ...
            # count += 1
            # IndexMaxErr = np.where(np.abs(Training_prederr) == np.amax(np.abs(Training_prederr)))
            # IndexMaxErr = IndexMaxErr[0][0]
            ...
            for K in range(len(Training_prederr)):
                ...
                elif (Training_Folders[K] in CEOptions['ImportantStr']) and np.abs(Training_prederr[K]) > CEOptions['GSError']:
            ...
            # for iwt in range(len(IndexMaxErr)):
            #     logfile.write("count, error, structure = {:>3d} {:>12.9f} {}\n".format(count, Training_prederr[IndexMaxErr[iwt]], Training_Folders[IndexMaxErr[iwt]]))
            # # end for-iwt
