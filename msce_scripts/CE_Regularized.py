...
from regression_utils_wocse import CE_Expansion_FitAll_CEREG
...
if __name__ == '__main__':
    ...
    CEOptions['LossFunction'] = 'CE-REG-PAIRS'
    CEOptions['TimeStr'] = timestr

    CEOptions['CALC-CV'] = 'TRUE'

    logfile = open("expansion_alpha-{}_{}.out".format(CEOptions['alpha'], timestr), "w+")
    logfile.write("\n")
    ...
    if os.path.exists("initial_params.out"):
        ...
        initial_CEI = np.zeros([np.sum(NumClusters)])
        for K in range(len(initial_CEI)): initial_CEI[K] += random.uniform(-0.001, 0.001)
        CEOptions["InitialFittingParams"] = initial_CEI
    ...
    CE_Expansion_FitAll_CEREG(LatticeInfo, ClusterCollection, cmd_correlation, CEOptions, logfile)
    logfile.write("\n")
    logfile.flush()
